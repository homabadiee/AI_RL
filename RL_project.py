...
def policy_iteration(env, custom_map, max_ittr=30, theta=0.01, discount_factor=0.9):
    ...
    # policy = convert_policy(env, policy)
    return V, policy
...
def first_visit_mc_prediction(env, policy, num_episodes, gamma):
    ...
    for i in range(num_episodes):
        ...
        for t in reversed(range(len(episode))):
            ...
            G = gamma * G + reward
            if state not in [e[0] for e in episode[:t]]:
                N[state] += 1
...
if __name__ == "__main__":
    ...
    V, policy = policy_iteration(env, map, max_ittr=30, theta=0.0001, discount_factor=0.9)
